...
//@des Remove a product from the cart
//@access Public
router.delete("/", async (req ,res) => {
  ...
  try{
    ...
    if (productIndex > -1){
      ...
      return res.status(404).json({ message: "Product not found in he cart"})
    }
  } catch(error) {
    console.log(error);
    return res.status(500).json({message: "Server Error"})
...
router.post("/merge", protect, async (req,res) => {
  ...
  try{
    //Find the guestcart and user cart
    ...
    if(guestCart){
      ...
      if(userCart){
        ...
        guestCart.products.forEach((guestItem) => {
          ...
          if(productIndex > -1){
            //If the items exists in the user cart, update the qunatity
            userCart.products[productIndex].quantity += guestItem.quantity;
          } else {
            //Otherwise, add te guest item to the cart
